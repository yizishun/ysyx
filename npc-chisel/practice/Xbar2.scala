...
class xbar2IO(xlen: Int) extends Bundle{
  val arb = new AXI4
  val uart = Flipped(new AXI4Lite(xlen))
  val clint = Flipped(new AXI4)
  val sram = Flipped(new AXI4Lite(xlen))
}

class Xbar2(val coreConfig: CoreConfig) extends Module{
  val io = IO(new xbar2IO(32))
  io.uart.clk := clock
  io.uart.rst := reset
  io.sram.clk := clock
  io.sram.rst := reset
